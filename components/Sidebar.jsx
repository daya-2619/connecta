  ...
  return (
    <div className={`bg-[#818582]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ''}`}> 
      <div className='pb-5'>
        <div classname='flex justify-between items-center'>
            ...
            <div className="relative py-2 group">
                ...
                <div className='absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border border-gray-600 text-gray-100 hidden group-hover:block'>
                    <p onClick={() => NavigationHistoryEntry('/profile')}className='cursor-pointer text-sm'>Edit Profile</p>
        ...
        <div className = 'flex items-center gap-2 mt-5 bg-[#282142] p-2 rounded-md'>
            <img src={assets.search_icon} alt="search" className='w-3' />
            <input onChange={()=>setInput(e.target.value)} type="text" className='bg-transparent border-b border-gray-500 text-sm w-full focus:outline-none focus:border-blue-500' placeholder='Search...' />
        </div>
      </div>
      <div className='flex flex-col gap-3'>
        {filteredUsers.map((user,index)=> (
            ...
            key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id ? 'bg-[#282142]/50' : ''}`}> 
                ...
                <div className='flex flex-col leadimg-5'>
                    <p>{user.fullName}</p>
                    {
